...
//Sets the Period of Data Transfer
uint16 initialMeasurementInterval = 1;
...
//            temperatureCelsius = (int32) ADCCountsCorrected;
            
//        	uint16 strain = ADC_GetResult16(0);
            /* Convert the ADC output to degrees Celsius. */
//            temperatureCelsius = (int32) ADCCountsCorrected;
...
//        counter = counter + 1;
//        temp = temp + counter;
//        float temp_tmp116 = tmp116_get_uint16();
        uint16 uint_tmp116 = tmp116_get_uint16();

        
//        float temp_tmp116 = 100;
...
//        temperatureValue = temperatureValue & 0x00FFFFFF;
        //temperatureValue = temp & 0x00FFFFFF;

        /* Copy temperature to array */
//        Set32ByPtr(temp_data + 1u, (uint32)temperatureValue);
//        Set32ByPtr(temp_data + 1u, (uint32)temp_tmp116);
        temp_data[0] = (uint8) 0;
        temp_data[1] = (uint8) uint_tmp116 & 0xff;
        temp_data[2] = (uint8) (uint_tmp116 >> 8u) & 0xff;
//        temp_data[1] = (uint8) strain & 0xff;
//        temp_data[2] = (uint8) (strain >> 8u) & 0xff;
        temp_data[3] = (uint8) 0;
        temp_data[4] = (uint8) 0;
