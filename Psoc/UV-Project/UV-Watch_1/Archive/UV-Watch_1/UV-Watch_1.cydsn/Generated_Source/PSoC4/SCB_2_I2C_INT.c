...
/*******************************************************************************
* Function Name: SCB_2_I2C_ISR
****************************************************************************//**
*
*  Handles the Interrupt Service Routine for the SCB I2C mode.
*
*******************************************************************************/
CY_ISR(SCB_2_I2C_ISR)
{
    uint32 diffCount;
    uint32 endTransfer;

#ifdef SCB_2_I2C_ISR_ENTRY_CALLBACK
    SCB_2_I2C_ISR_EntryCallback();
#endif /* SCB_2_I2C_ISR_ENTRY_CALLBACK */

#if (SCB_2_I2C_CUSTOM_ADDRESS_HANDLER_CONST)
    uint32 response;

    response = SCB_2_I2C_ACK_ADDR;
#endif /* (SCB_2_I2C_CUSTOM_ADDRESS_HANDLER_CONST) */

    endTransfer = 0u; /* Continue active transfer */

    /* Calls customer routine if registered */
    if(NULL != SCB_2_customIntrHandler)
    {
        SCB_2_customIntrHandler();
    }

    if(SCB_2_CHECK_INTR_I2C_EC_MASKED(SCB_2_INTR_I2C_EC_WAKE_UP))
    {
    ...
    */
    if(SCB_2_CHECK_I2C_FSM_MASTER)
    {
        #if(SCB_2_I2C_MASTER)
        {
            ...
            */
            if(SCB_2_CHECK_INTR_MASTER_MASKED(SCB_2_INTR_MASTER_I2C_BUS_ERROR))
            {
                SCB_2_mstrStatus |= (uint16) (SCB_2_I2C_MSTAT_ERR_XFER |
                                                         SCB_2_I2C_MSTAT_ERR_BUS_ERROR);

                endTransfer = SCB_2_I2C_CMPLT_ANY_TRANSFER;
            }
            ...
            */
            if(SCB_2_CHECK_INTR_MASTER_MASKED(SCB_2_INTR_MASTER_I2C_ARB_LOST))
            {
                SCB_2_mstrStatus |= (uint16) (SCB_2_I2C_MSTAT_ERR_XFER |
                                                         SCB_2_I2C_MSTAT_ERR_ARB_LOST);

                endTransfer = SCB_2_I2C_CMPLT_ANY_TRANSFER;
            }

            #if(SCB_2_I2C_MULTI_MASTER_SLAVE)
            {
                /* I2C_MASTER_CMD_M_START_ON_IDLE:
                * MultiMaster-Slave does not generate start, because Slave was addressed.
                * Pass control to slave.
                */
                if(SCB_2_CHECK_I2C_MASTER_CMD(SCB_2_I2C_MASTER_CMD_M_START_ON_IDLE))
                {
                    SCB_2_mstrStatus |= (uint16) (SCB_2_I2C_MSTAT_ERR_XFER |
                                                             SCB_2_I2C_MSTAT_ERR_ABORT_XFER);
            ...
            */
            if(0u != endTransfer)
            {
                ...
                SCB_2_mstrStatus |= (uint16) SCB_2_GET_I2C_MSTAT_CMPLT;

                #if(SCB_2_I2C_MULTI_MASTER_SLAVE)
                {
                    if(SCB_2_CHECK_I2C_FSM_ADDR)
                    {
                        ...
                        */
                        if(SCB_2_CHECK_I2C_MASTER_CMD(SCB_2_I2C_MASTER_CMD_M_START_ON_IDLE))
                        {
                            SCB_2_I2C_MASTER_CLEAR_START;

                            endTransfer = SCB_2_I2C_CMPLT_ANY_TRANSFER; /* Pass control to Slave */
                        }
                        ...
                        */
                        else if((!SCB_2_CHECK_INTR_SLAVE_MASKED(SCB_2_INTR_SLAVE_I2C_BUS_ERROR))
                               && SCB_2_CHECK_INTR_MASTER_MASKED(SCB_2_INTR_MASTER_I2C_ARB_LOST))
                        {
                            endTransfer = SCB_2_I2C_CMPLT_ANY_TRANSFER; /* Pass control to Slave */
                        }
                        else
                        ...
                        {
                            SCB_2_CLEAR_TX_FIFO; /* Shifter keeps address, clear it */

                            SCB_2_DISABLE_MASTER_AUTO_DATA_ACK; /* In case of reading disable autoACK */
                            ...
                            SCB_2_SetRxInterruptMode(SCB_2_NO_INTR_SOURCES);
                        ...
                        else
                        {
                            ...
                            SCB_2_state = SCB_2_I2C_FSM_EXIT_IDLE;
                        }
                    }
                    else
                    {
    ...
    else /* (SCB_2_CHECK_I2C_FSM_SLAVE) */
    {
        #if(SCB_2_I2C_SLAVE)
        {
            /* INTR_SLAVE_I2C_BUS_ERROR or SCB_2_INTR_SLAVE_I2C_ARB_LOST:
            * A Misplaced Start or Stop condition occurred on the bus: set a flag
            * to notify an error condition.
            */
            if(SCB_2_CHECK_INTR_SLAVE_MASKED(SCB_2_INTR_SLAVE_I2C_BUS_ERROR |
                                                        SCB_2_INTR_SLAVE_I2C_ARB_LOST))
            {
                if(SCB_2_CHECK_I2C_FSM_RD)
                {
                    /* TX direction: master reads from slave */
                    SCB_2_slStatus &= (uint8) ~SCB_2_I2C_SSTAT_RD_BUSY;
                    SCB_2_slStatus |= (uint8) (SCB_2_I2C_SSTAT_RD_ERR |
                                                          SCB_2_I2C_SSTAT_RD_CMPLT);
                }
                else
                {
                    /* RX direction: master writes into slave */
                    SCB_2_slStatus &= (uint8) ~SCB_2_I2C_SSTAT_WR_BUSY;
                    SCB_2_slStatus |= (uint8) (SCB_2_I2C_SSTAT_WR_ERR |
                                                          SCB_2_I2C_SSTAT_WR_CMPLT);
                }
    ...
    */
    ...
    if(SCB_2_CHECK_I2C_FSM_MASTER)
    {
        #if(SCB_2_I2C_MASTER)
        {
            ...
            */
            if(SCB_2_CHECK_INTR_MASTER_MASKED(SCB_2_INTR_MASTER_I2C_STOP))
            {
            ...
            {
                if(SCB_2_CHECK_I2C_FSM_ADDR) /* Address stage */
                {
                    /* INTR_MASTER_I2C_NACK:
                    * The master sent an address but it was NACKed by the slave. Complete transaction.
                    */
                    if(SCB_2_CHECK_INTR_MASTER_MASKED(SCB_2_INTR_MASTER_I2C_NACK))
                    {
                        SCB_2_ClearMasterInterruptSource(SCB_2_INTR_MASTER_I2C_NACK);

                        SCB_2_mstrStatus |= (uint16) (SCB_2_I2C_MSTAT_ERR_XFER |
                                                                 SCB_2_I2C_MSTAT_ERR_ADDR_NAK);
                    ...
                    */
                    ...
                    {
                        if(SCB_2_CHECK_I2C_FSM_RD) /* Reading */
                        {
                        ...
                        else /* Writing */
                        {
                            ...
                            if(0u != SCB_2_mstrWrBufSize)
                            {
                                /* Enable INTR.TX_EMPTY if there is data to transmit */
                                SCB_2_SetTxInterruptMode(SCB_2_INTR_TX_EMPTY);
                            }
                        }
                    }
                }

                if(SCB_2_CHECK_I2C_FSM_DATA) /* Data phase */
                {
                    if(SCB_2_CHECK_I2C_FSM_RD) /* Reading */
                    {
                        /* INTR_RX_FULL:
                        ...
                        */
                        if(SCB_2_CHECK_INTR_RX_MASKED(SCB_2_INTR_RX_FULL))
                        {
                            /* Calculate difference */
                            diffCount =  SCB_2_mstrRdBufSize -
                                        (SCB_2_mstrRdBufIndex + SCB_2_GET_RX_FIFO_ENTRIES);

                            /* Proceed transaction or end it when RX FIFO becomes FULL again */
                            if(diffCount > SCB_2_I2C_FIFO_SIZE)
                            {
                                diffCount = SCB_2_I2C_FIFO_SIZE;
                            }
                            else
                            {
                                if(0u == diffCount)
                                {
                                    SCB_2_DISABLE_MASTER_AUTO_DATA_ACK;
                            ...
                            {
                                SCB_2_mstrRdBufPtr[SCB_2_mstrRdBufIndex] = (uint8)
                                                                                        SCB_2_RX_FIFO_RD_REG;
                                SCB_2_mstrRdBufIndex++;
                            }
                        }
                        /* INTR_RX_NOT_EMPTY:
                        ...
                        */
                        else if(SCB_2_CHECK_INTR_RX_MASKED(SCB_2_INTR_RX_NOT_EMPTY))
                        {
                            /* Put data in component buffer */
                            SCB_2_mstrRdBufPtr[SCB_2_mstrRdBufIndex] = (uint8) SCB_2_RX_FIFO_RD_REG;
                            SCB_2_mstrRdBufIndex++;

                            if(SCB_2_mstrRdBufIndex < SCB_2_mstrRdBufSize)
                            {
                        ...
                        SCB_2_ClearRxInterruptSource(SCB_2_INTR_RX_ALL);
                    }
                    else /* Writing */
                    {
                        /* INTR_MASTER_I2C_NACK :
                        ...
                        */
                        if(SCB_2_CHECK_INTR_MASTER_MASKED(SCB_2_INTR_MASTER_I2C_NACK))
                        {
                            ...
                            SCB_2_mstrWrBufIndexTmp -= (SCB_2_GET_TX_FIFO_ENTRIES +
                                                                   SCB_2_GET_TX_FIFO_SR_VALID);

                            /* Update number of transferred bytes */
                            SCB_2_mstrWrBufIndex = SCB_2_mstrWrBufIndexTmp;

                            SCB_2_mstrStatus |= (uint16) (SCB_2_I2C_MSTAT_ERR_XFER |
                                                                     SCB_2_I2C_MSTAT_ERR_SHORT_XFER);
                        ...
                        */
                        else if(SCB_2_CHECK_INTR_TX_MASKED(SCB_2_INTR_TX_EMPTY))
                        {
                            while(SCB_2_I2C_FIFO_SIZE != SCB_2_GET_TX_FIFO_ENTRIES)
                            {
                                ...
                                */
                                if(SCB_2_mstrWrBufIndexTmp < SCB_2_mstrWrBufSize)
                                {
                                    ...
                                    */
                                    if(SCB_2_mstrWrBufIndexTmp == (SCB_2_mstrWrBufSize - 1u))
                                    {
                                    ...
                                    SCB_2_TX_FIFO_WR_REG = (uint32) SCB_2_mstrWrBufPtr[SCB_2_mstrWrBufIndexTmp];
                                    SCB_2_mstrWrBufIndexTmp++;
                                }
                                else
                                {
                            ...
                            SCB_2_ClearTxInterruptSource(SCB_2_INTR_TX_ALL);
                        #else
                            SCB_2_ClearTxInterruptSource(SCB_2_INTR_TX_EMPTY);
                        #endif /* (SCB_2_CY_SCBIP_V0) */
                        }
                        /* INTR_TX_UNDERFLOW:
                        ...
                        */
                        else if(SCB_2_CHECK_INTR_TX_MASKED(SCB_2_INTR_TX_UNDERFLOW))
                        {
                            /* Update number of transferred bytes */
                            SCB_2_mstrWrBufIndex = SCB_2_mstrWrBufIndexTmp;
                ...
                if(0u != endTransfer) /* Complete transfer */
                {
                    ...
                    if(SCB_2_CHECK_I2C_MODE_NO_STOP(SCB_2_mstrControl))
                    {
    ...
    } /* (SCB_2_CHECK_I2C_FSM_MASTER) */


    /* FSM Slave */
    else if(SCB_2_CHECK_I2C_FSM_SLAVE)
    {
        #if(SCB_2_I2C_SLAVE)
        {
            /* INTR_SLAVE_NACK:
            ...
            */
            if(SCB_2_CHECK_INTR_SLAVE_MASKED(SCB_2_INTR_SLAVE_I2C_NACK))
            {
                SCB_2_ClearSlaveInterruptSource(SCB_2_INTR_SLAVE_I2C_NACK);

                /* All entries that remain in TX FIFO are: FIFO Size + 1 (SHIFTER) */
                diffCount = (SCB_2_GET_TX_FIFO_ENTRIES + SCB_2_GET_TX_FIFO_SR_VALID);

                if(SCB_2_slOverFlowCount > diffCount) /* Overflow */
                {
                    SCB_2_slStatus |= (uint8) SCB_2_I2C_SSTAT_RD_OVFL;
                }
                else /* No Overflow */
                {
                    /* Roll-back temporary index */
                    SCB_2_slRdBufIndexTmp -= (diffCount - SCB_2_slOverFlowCount);
                }

                /* Update slave of transferred bytes */
                SCB_2_slRdBufIndex = SCB_2_slRdBufIndexTmp;
                ...
                SCB_2_slStatus &= (uint8) ~SCB_2_I2C_SSTAT_RD_BUSY;
                SCB_2_slStatus |= (uint8)  SCB_2_I2C_SSTAT_RD_CMPLT;
                SCB_2_state     =  SCB_2_I2C_FSM_IDLE;
                
            #ifdef SCB_2_I2C_SLAVE_CMPLT_CALLBACK
                /* Read complete */
                SCB_2_I2C_SlaveCompleteCallback();
            #endif /* SCB_2_I2C_SLAVE_CMPLT_CALLBACK */
            }
            ...
            */
            if(SCB_2_CHECK_INTR_SLAVE_MASKED(SCB_2_INTR_SLAVE_I2C_WRITE_STOP))
            {
                SCB_2_ClearSlaveInterruptSource(SCB_2_INTR_SLAVE_I2C_WRITE_STOP);
                ...
                * I2C_ADDR_MATCH.
                */
                if (0u != (SCB_2_I2C_CTRL_REG & SCB_2_I2C_CTRL_S_READY_DATA_ACK))
                {
                    while(0u != SCB_2_GET_RX_FIFO_ENTRIES)
                    {
                        #if(SCB_2_CHECK_I2C_ACCEPT_ADDRESS)
                        {
                            if((1u == SCB_2_GET_RX_FIFO_ENTRIES) &&
                               (SCB_2_CHECK_INTR_SLAVE_MASKED(SCB_2_INTR_SLAVE_I2C_ADDR_MATCH)))
                            {
                        ...
                        /* Put data in component buffer */
                        SCB_2_slWrBufPtr[SCB_2_slWrBufIndex] = (uint8) SCB_2_RX_FIFO_RD_REG;
                        SCB_2_slWrBufIndex++;
                    }

                    SCB_2_DISABLE_SLAVE_AUTO_DATA;
                }

                if(SCB_2_CHECK_INTR_RX(SCB_2_INTR_RX_OVERFLOW))
                {
                    SCB_2_slStatus |= (uint8) SCB_2_I2C_SSTAT_WR_OVFL;
                }
                ...
                SCB_2_slStatus &= (uint8) ~SCB_2_I2C_SSTAT_WR_BUSY;
                SCB_2_slStatus |= (uint8)  SCB_2_I2C_SSTAT_WR_CMPLT;
                SCB_2_state     =  SCB_2_I2C_FSM_IDLE;
            ...
            */
        ...
        #endif /* (SCB_2_I2C_CUSTOM_ADDRESS_HANDLER_CONST) */
            {
                ...
                SCB_2_ClearI2CExtClkInterruptSource(SCB_2_INTR_I2C_EC_WAKE_UP);

                #if (SCB_2_I2C_CUSTOM_ADDRESS_HANDLER)
                {
                    if (NULL != SCB_2_customAddressHandler)
                    {
                        /* Call custom address handler */
                        response = SCB_2_customAddressHandler();
                    }
                    else
                    {
                        /* Read address from the RX FIFO. If there is no address underflow triggers but
                        * component does not use that source. */
                        (void) SCB_2_RX_FIFO_RD_REG;
                        response = SCB_2_I2C_ACK_ADDR;
                    }
            ...
            #if (SCB_2_I2C_CUSTOM_ADDRESS_HANDLER_CONST)
                if (response == SCB_2_I2C_NAK_ADDR)
                {
                #if (!SCB_2_CY_SCBIP_V0)
                    /* Disable write stop interrupt source as it triggers after address was NACKed. Ticket ID#156094 */
                    SCB_2_DISABLE_INTR_SLAVE(SCB_2_INTR_SLAVE_I2C_WRITE_STOP);
                #endif /* (!SCB_2_CY_SCBIP_V0) */

                    /* Clear address match and stop history */
                    SCB_2_ClearSlaveInterruptSource(SCB_2_INTR_SLAVE_ALL);

                    /* ACK the address byte */
                    SCB_2_I2C_SLAVE_GENERATE_NACK;
                }
                else
            #endif /* (SCB_2_I2C_CUSTOM_ADDRESS_HANDLER_CONST) */
                {
                    ...
                    /* TX direction: master reads from slave */
                    {
                        ...
                        SCB_2_slRdBufIndexTmp = SCB_2_slRdBufIndex;

                        /* Start master reading */
                        SCB_2_slStatus |= (uint8) SCB_2_I2C_SSTAT_RD_BUSY;
                        SCB_2_state     = SCB_2_I2C_FSM_SL_RD;
                    }
                    else
                    /* RX direction: master writes into slave */
                    {
                        /* Calculate available buffer size */
                        diffCount = (SCB_2_slWrBufSize - SCB_2_slWrBufIndex);
                        ...
                        /* Receive data: into RX FIFO */
                        {
                            if(diffCount == SCB_2_I2C_FIFO_SIZE)
                            {
                                ...
                                SCB_2_ENABLE_SLAVE_AUTO_DATA;
                            }
                            else
                            {
                                /* Stretch clock when RX FIFO becomes FULL */
                                SCB_2_ENABLE_SLAVE_AUTO_DATA_ACK;
                        ...
                        /* Start master reading */
                        SCB_2_slStatus |= (uint8) SCB_2_I2C_SSTAT_WR_BUSY;
                        SCB_2_state     = SCB_2_I2C_FSM_SL_WR;
                    }
                    ...
                    SCB_2_ENABLE_INTR_SLAVE(SCB_2_INTR_SLAVE_I2C_WRITE_STOP);
                #endif /* (!SCB_2_CY_SCBIP_V0) */

                    /* ACK the address byte */
                    SCB_2_I2C_SLAVE_GENERATE_ACK;
                }
            }
            ...
            */
            if(SCB_2_CHECK_INTR_RX_MASKED(SCB_2_INTR_RX_FULL))
            {
                ...
                diffCount =  SCB_2_slWrBufSize -
                            (SCB_2_slWrBufIndex + SCB_2_I2C_FIFO_SIZE);

                if(diffCount > SCB_2_I2C_FIFO_SIZE) /* Proceed transaction */
                {
                    diffCount   = SCB_2_I2C_FIFO_SIZE;
                    endTransfer = 0u;  /* Continue active transfer */
                }
                ...
                {
                    SCB_2_ENABLE_SLAVE_AUTO_DATA_NACK;
                ...
                SCB_2_ClearRxInterruptSource(SCB_2_INTR_RX_FULL);
            }
            ...
            */
            ...
            {
                diffCount = SCB_2_RX_FIFO_RD_REG;

                if(SCB_2_slWrBufIndex < SCB_2_slWrBufSize)
                {
                    SCB_2_I2C_SLAVE_GENERATE_ACK;

                    /* Put data into component buffer */
                    SCB_2_slWrBufPtr[SCB_2_slWrBufIndex] = (uint8) diffCount;
                    SCB_2_slWrBufIndex++;
                }
            ...
            */
            if(SCB_2_CHECK_INTR_TX_MASKED(SCB_2_INTR_TX_EMPTY))
            {
                while(SCB_2_I2C_FIFO_SIZE != SCB_2_GET_TX_FIFO_ENTRIES)
                {
                    ...
                    */
                    if(SCB_2_slRdBufIndexTmp < SCB_2_slRdBufSize)
                    /* Data from buffer */
                    {
                        SCB_2_TX_FIFO_WR_REG = (uint32) SCB_2_slRdBufPtr[SCB_2_slRdBufIndexTmp];
                        SCB_2_slRdBufIndexTmp++;
                    }
                    else
                    /* Probably Overflow */
                    {
                        SCB_2_TX_FIFO_WR_REG = SCB_2_I2C_SLAVE_OVFL_RETURN;

                        if(SCB_2_slOverFlowCount <= SCB_2_I2C_TX_OVERFLOW_COUNT)
                        {
                            /* Get counter in range of overflow. */
                            SCB_2_slOverFlowCount++;
                        }
                    }
                }
...
#ifdef SCB_2_I2C_ISR_EXIT_CALLBACK
    SCB_2_I2C_ISR_ExitCallback();
#endif /* SCB_2_I2C_ISR_EXIT_CALLBACK */
