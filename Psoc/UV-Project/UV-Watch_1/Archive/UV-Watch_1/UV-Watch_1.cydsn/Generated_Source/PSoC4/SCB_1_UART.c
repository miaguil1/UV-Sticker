...
#include "cyapicallbacks.h"
    ...
    ***************************************/
    ...
    const SCB_1_UART_INIT_STRUCT SCB_1_configUart =
    {
        ...
        (uint8) SCB_1_UART_RX_BREAK_WIDTH
    };
    ...
    void SCB_1_UartInit(const SCB_1_UART_INIT_STRUCT *config)
    {
        ...
        else
        {
            ...
            SCB_1_UART_RX_CTRL_REG = SCB_1_GET_UART_RX_CTRL_MODE(config->stopBits)              |
                                        ...
                                        SCB_1_GET_UART_RX_CTRL_BREAK_WIDTH(config->breakWidth);
...
void SCB_1_UartSetRxAddressMask(uint32 addressMask)
{
    ...
    *******************************************************************************/
    ...
    /*******************************************************************************
    * Function Name: SCB_1_UartSendBreakBlocking
    ****************************************************************************//**
    *
    ...
    *
    *******************************************************************************/
    void SCB_1_UartSendBreakBlocking(uint32 breakWidth)
    {
        uint32 txCtrlReg;
        uint32 txIntrReg;

        /* Disable all UART TX interrupt source and clear UART TX Done history */
        txIntrReg = SCB_1_GetTxInterruptMode();
        SCB_1_SetTxInterruptMode(0u);
        SCB_1_ClearTxInterruptSource(SCB_1_INTR_TX_UART_DONE);

        /* Store TX CTRL configuration */
        txCtrlReg = SCB_1_TX_CTRL_REG;

        /* Set break width */
        SCB_1_TX_CTRL_REG = (SCB_1_TX_CTRL_REG & (uint32) ~SCB_1_TX_CTRL_DATA_WIDTH_MASK) |
                                        SCB_1_GET_TX_CTRL_DATA_WIDTH(breakWidth);

        /* Generate break */
        SCB_1_TX_FIFO_WR_REG = 0u;

        /* Wait for break completion */
        while (0u == (SCB_1_GetTxInterruptSource() & SCB_1_INTR_TX_UART_DONE))
        {
        }
        ...
        SCB_1_TX_CTRL_REG = txCtrlReg;
        SCB_1_SetTxInterruptMode(txIntrReg);
    }
