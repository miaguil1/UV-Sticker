...
void CyBLE_Uart_Start (void)
{
        ...
        /* Setup ISR */
        BLE_1_uart_isr_StartEx(&CyBLE_Uart_Interrupt);

        BLE_1_HAL_Uart_Start();
...
{
    #if (CYBLE_HCI_TYPE == CYBLE_HCI_OVER_UART)
        BLE_1_HAL_Uart_SpiUartPutArray(dataBuf, (uint32)length);
    #endif /* (CYBLE_HCI_TYPE == CYBLE_HCI_OVER_UART) */
}
...
/* Interface to BLE_1_bless_isr_Enable function */
void CyBleHal_EnableGlobalInterrupts(void)
{
    BLE_1_bless_isr_Enable();
}

/* Interface to BLE_1_bless_isr_Disable function */
...
void CYBLE_BlessStart(void)
{
    ...
    BLE_1_bless_isr_StartEx(&CyBLE_Bless_Interrupt);
}
